 ...
 */
export function getFiveElementBureau(lifePalaceStem, lifePalaceBranch) {
    const stemBranch = `${lifePalaceStem}${lifePalaceBranch}`;
    const bureauInfo = bureauData.nayin[stemBranch];
    if (!bureauInfo) {
 ...
 * 從某地支開始，順時針移動n個位置
 */
function moveClockwise(start, steps) {
 ...
 * 從某地支開始，逆時針移動n個位置
 */
function moveCounterClockwise(start, steps) {
    const startIndex = branchToIndex(start);
    return indexToBranch(startIndex - steps);
}
/**
 ...
 * @returns 紫微星所在地支
 */
export function findZiweiPosition(bureau, lunarDay) {
 ...
 * @returns 天府星所在地支
 */
export function findTianfuPosition(ziweiPosition) {
    const tianfuPosition = ziweiTianfuMap.ziweiToTianfu[ziweiPosition];
    if (!tianfuPosition) {
        throw new Error(`Invalid ziwei position: ${ziweiPosition}`);
    }
    return tianfuPosition;
}
/**
 ...
 * @returns 紫微星系各星的位置
 */
export function placeZiweiSystemStars(ziweiPosition) {
    const positions = {};
 ...
 * @returns 天府星系各星的位置
 */
export function placeTianfuSystemStars(tianfuPosition) {
    const positions = {};
 ...
 */
export function addStarBrightness(starName, branch) {
    const brightness = starsMainData.brightness[starName];
    if (!brightness) {
        // 如果不是14主星，返回無狀態
        return { name: starName };
    }
    const status = brightness[branch];
    return {
        name: starName,
 ...
 * @returns 每個地支對應的主星列表
 */
export function placeAllMainStars(ziweiPosition, tianfuPosition) {
    // 初始化12個地支的星曜列表
    const starMap = {
        子: [], 丑: [], 寅: [], 卯: [], 辰: [], 巳: [],
        午: [], 未: [], 申: [], 酉: [], 戌: [], 亥: []
