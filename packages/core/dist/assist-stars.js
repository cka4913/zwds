 ...
 * @returns 目標地支
 */
function moveClockwise(branch, steps) {
 ...
 * @returns 目標地支
 */
function moveCounterClockwise(branch, steps) {
    const index = getBranchIndex(branch);
 ...
 */
function addStarBrightness(starName, branch) {
 ...
 * @returns 文昌星所在地支
 */
export function placeWenChang(hourBranch) {
    // 從戌宮起子時，逆時針數至出生時辰
    // 戌 = index 10
    const xuIndex = 10; // 戌
    const ziIndex = 0; // 子
 ...
 * @returns 文曲星所在地支
 */
export function placeWenQu(hourBranch) {
    // 從辰宮起子時，順時針數至出生時辰
    // 辰 = index 4
    const chenIndex = 4; // 辰
    const ziIndex = 0; // 子
 ...
 * @returns 左輔星所在地支
 */
export function placeZuoFu(lunarMonth) {
    // 從辰宮起正月，順時針數至出生月份
    // 辰 = index 4
    const chenIndex = 4; // 辰
    // 正月 = 1，在辰宮（index 4）
 ...
 * @returns 右弼星所在地支
 */
export function placeYouBi(lunarMonth) {
    // 從戌宮起正月，逆時針數至出生月份
    // 戌 = index 10
    const xuIndex = 10; // 戌
    // 正月 = 1，在戌宮（index 10）
 ...
 * @returns 天魁和天鉞所在地支
 */
export function placeTianKuiTianYue(yearStem) {
    // 天魁天鉞對照表
    const tianKuiYueMap = {
        "甲": { kui: "丑", yue: "未" },
        "乙": { kui: "子", yue: "申" },
 ...
 * @returns 祿存星所在地支
 */
export function placeLuCun(yearStem) {
    const luCunMap = {
        "甲": "寅",
        "乙": "卯",
        "丙": "巳",
 ...
 * @returns 擎羊和陀羅所在地支
 */
export function placeQingYangTuoLuo(yearStem) {
 ...
 * @returns 火星和鈴星所在地支
 */
export function placeHuoXingLingXing(yearBranch, hourBranch) {
    // 火星鈴星對照表（根據年支組合）
    // 寅午戌年：火星從丑起子時順行，鈴星從卯起子時順行
    // 申子辰年：火星從寅起子時順行，鈴星從戌起子時順行
    // 巳酉丑年：火星從卯起子時順行，鈴星從戌起子時順行
    // 亥卯未年：火星從酉起子時順行，鈴星從戌起子時順行
    const huoXingStart = {
        "寅午戌": 1, // 丑
        "申子辰": 2, // 寅
        "巳酉丑": 3, // 卯
 ...
 * @returns 地空和地劫所在地支
 */
export function placeDiKongDiJie(hourBranch) {
    // 從亥宮起子時
    const haiIndex = 11; // 亥
    const ziIndex = 0; // 子
    // 計算時辰偏移
 ...
 * @returns 天馬星所在地支
 */
export function placeTianMa(yearBranch) {
    const tianMaMap = {
        "寅": "申", "午": "申", "戌": "申",
        "申": "寅", "子": "寅", "辰": "寅",
        "巳": "亥", "酉": "亥", "丑": "亥",
 ...
 * @returns 每個地支對應的輔星列表
 */
export function placeAllAssistStars(yearStem, yearBranch, lunarMonth, hourBranch) {
    // 初始化12個地支的輔星列表
    const assistStarMap = {
        子: [], 丑: [], 寅: [], 卯: [], 辰: [], 巳: [],
        午: [], 未: [], 申: [], 酉: [], 戌: [], 亥: []
