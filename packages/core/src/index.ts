...
export function makeChart(meta: ChartMeta): ZwdsChart {
  ...
  const doujunBranch = getDoujunBranch(
    lunar.month,
    hourBranch,
    yearStemBranch.branch,
    currentYearStemBranch.branch
  );
...
export function renderText(chart: ZwdsChart): string {
  ...
  // 宫位简称映射
  const palaceAbbrev: Record<string, string> = {
  ...
  // 计算当前宫位在每个流层中的位置
  function getLayerPalace(currentPalaceName: PalaceName, layerEntryPalace: PalaceName | undefined): string {
    if (!layerEntryPalace) return "";
  ...
  // 找出各个流层的命宫
  let decadePalaceName: PalaceName | undefined;
  let annualPalaceName: PalaceName | undefined;
  let monthlyPalaceName: PalaceName | undefined;
  let dailyPalaceName: PalaceName | undefined;
  let hourlyPalaceName: PalaceName | undefined;

  for (const pName of palaceOrder) {
    const p = chart.palaces[pName];
    if (p?.isDecadePalace) decadePalaceName = pName;
    if (p?.isAnnualPalace) annualPalaceName = pName;
    if (p?.isMonthlyPalace) monthlyPalaceName = pName;
    if (p?.isDailyPalace) dailyPalaceName = pName;
    if (p?.isHourlyPalace) hourlyPalaceName = pName;
  }
