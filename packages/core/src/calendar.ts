 ...
 */
export function parseISODateTime(iso: string): SolarDate {
 ...
 */
export function getYearStemBranch(year: number): { stem: HeavenlyStem; branch: EarthBranch } {
  // 天干: (year - 4) % 10
  // 地支: (year - 4) % 12
...
): { stem: HeavenlyStem; branch: EarthBranch } {
 ...
 */
export function getDayStemBranch(solar: SolarDate): { stem: HeavenlyStem; branch: EarthBranch } {
  const solarDate = Solar.fromYmd(solar.year, solar.month, solar.day);
  const lunar = solarDate.getLunar();
  ...
  if (dayGanZhi && dayGanZhi.length === 2) {
    ...
    const branch = dayGanZhi[1] as EarthBranch;
    return { stem, branch };
  }
 ...
 */
export function getHourBranch(hour: number): EarthBranch {
  const hourRanges = [
 ...
 * 戊癸何方發（戊日、癸日的子時為壬子時）
 */
export function getHourStem(hourBranch: EarthBranch, dayStem: HeavenlyStem): HeavenlyStem {
 ...
 * 格式化時辰地支為中文時辰名
 */
export function getHourName(branch: EarthBranch): string {
  const hourNames: Record<EarthBranch, string> = {
    "子": "子",
    "丑": "丑",
    "寅": "寅",
    "卯": "卯",
