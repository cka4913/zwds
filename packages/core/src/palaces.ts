 ...
 * @returns 命宮地支
 */
export function findLifePalaceBranch(lunarMonth: number, hourBranch: EarthBranch): EarthBranch {
  // Step 1: 從寅宮起正月，順數到生月
 ...
 * @returns 身宮地支
 */
export function findBodyPalaceBranch(lunarMonth: number, hourBranch: EarthBranch): EarthBranch {
 ...
 * @returns 命宮天干
 */
export function findLifePalaceStem(yearStem: HeavenlyStem, lifePalaceBranch: EarthBranch): HeavenlyStem {
 ...
 * @returns 每個宮位對應的地支
 */
export function arrangePalaces(lifePalaceBranch: EarthBranch): Record<PalaceName, EarthBranch> {
  const result = {} as Record<PalaceName, EarthBranch>;
  const lifePalaceIndex = getBranchIndex(lifePalaceBranch);
 ...
 * @returns 每個宮位的天干
 */
export function arrangePalaceStems(
  yearStem: HeavenlyStem,
  palaceBranches: Record<PalaceName, EarthBranch>,
  _sex?: Sex // 保留參數以兼容現有調用，但不再使用
): Record<PalaceName, HeavenlyStem> {
  const result = {} as Record<PalaceName, HeavenlyStem>;
 ...
 * （此函數用於除錯驗證）
 */
export function validatePalacePositions(
